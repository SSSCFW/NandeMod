...
public class BlockOldFiller extends BlockContainer {
	...
	public BlockOldFiller() {
		super(Material.iron);
		setBlockName("oldFiller");
		setHardness(1.5F);
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		return new TileOldFiller();
	}

	@Override
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int meta, float hitX, float hitY, float hitZ) {
		if(!world.isRemote) {
			player.openGui(NandeMod.instance, Constants.GUI_ID_OLD_FILLER, world, x, y, z);
		}
		return true;
	}
	...
	public IIcon getIcon(IBlockAccess iBlockAccess, int x, int y, int z, int side) {
		...
		if(tileEntity != null && tileEntity instanceof TileOldFiller) {
			TileOldFiller fillerEX = (TileOldFiller)tileEntity;
			if(side == 1 || side == 0) {
	...
	public void registerBlockIcons(IIconRegister register) {
		front = register.registerIcon(NandeMod.MODID + ":filler_front");
		top = register.registerIcon(NandeMod.MODID + ":filler_top");
		top_On = register.registerIcon(NandeMod.MODID + ":filler_top_on");
	}

	@Override
	public void breakBlock(World world, int x, int y, int z, Block block, int notice) {
		TileOldFiller fillerEX = (TileOldFiller)world.getTileEntity(x, y, z);
		if(fillerEX != null) {
