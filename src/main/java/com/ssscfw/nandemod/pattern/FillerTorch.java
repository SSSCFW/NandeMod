...
public class FillerTorch extends FillerPatternCore{
	...
	protected boolean set(TileOldFiller fillerEX, ItemStack stack) {
		if(stack == null) {
			return false;
		}
		while(fillerEX.cz >= fillerEX.sz && fillerEX.cz <= fillerEX.ez) {
			while(fillerEX.cx >= fillerEX.sx && fillerEX.cx <= fillerEX.ex) {
				if(isFilling(fillerEX.getWorldObj(), fillerEX.cx, fillerEX.cy, fillerEX.cz) && Math.abs(fillerEX.cx - fillerEX.mx) % Alldata.intervalTorch == 0 && Math.abs(fillerEX.cz - fillerEX.mz) % Alldata.intervalTorch == 0) {
