...
public abstract class FillerPatternCore{
	...
	public boolean iteratePattern(TileEntity fillerEX, IBox box, ItemStack stack) {
		...
		if(fillerEX instanceof TileOldFiller) {
			return iteratePattern((TileOldFiller)fillerEX, stack);
		}
		return false;
	}

	public abstract boolean iteratePattern(TileOldFiller fillerEX, ItemStack stack);

	protected boolean fill(TileOldFiller fillerEX, ItemStack stack) {
		return fill(fillerEX.sx, fillerEX.sy, fillerEX.sz, fillerEX.ex, fillerEX.ey, fillerEX.ez, fillerEX, stack);
	}

	protected boolean fill(int sx, int sy, int sz, int ex, int ey, int ez, TileOldFiller fillerEX, ItemStack stack) {
		if(stack == null) {
			return false;
		}
	...
	protected boolean fill2(TileOldFiller fillerEX, ItemStack stack) {
		if(stack == null) {
			return false;
		}
		int y = fillerEX.sy - 1;
	...
	protected boolean emptyUD(TileOldFiller fillerEX, ItemStack stack) {
	...
	protected boolean emptyDU(TileOldFiller fillerEX, ItemStack stack) {
		while(fillerEX.cy >= fillerEX.sy && fillerEX.cy <= fillerEX.ey && fillerEX.cy <= 255 && fillerEX.cy > 0) {
	...
	protected boolean erase(TileOldFiller fillerEX, ItemStack stack) {
		while(fillerEX.cy >= fillerEX.sy && fillerEX.cy <= fillerEX.ey && fillerEX.cy <= 255 && fillerEX.cy > 0) {
			while(fillerEX.cz >= fillerEX.sz && fillerEX.cz <= fillerEX.ez) {
				while(fillerEX.cx >= fillerEX.sx && fillerEX.cx <= fillerEX.ex) {
					if(isErase(fillerEX)) {
	...
	protected boolean isErase(TileOldFiller fillerEX) {
		Block block = fillerEX.getWorldObj().getBlock(fillerEX.cx, fillerEX.cy, fillerEX.cz);
		return block != Blocks.air && block != Alldata.blocks.get("old_filler") && block != Blocks.end_portal && block != Blocks.end_portal_frame;
	}

	protected void eraseBlock(World world, int x, int y, int z) {
		Alldata.setBlockToAir(world, x, y, z);
	...
	protected void setBlock(TileOldFiller fillerEX, int x, int y, int z, ItemStack stack) {
